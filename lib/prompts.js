...
const BASE = `You are drafting a reply on behalf of the user.

LANGUAGE RULE (highest priority, overrides every other instruction below): Detect the language of the target post and reply in EXACTLY that language. If the target is in Greek, reply in Greek. If it is in Spanish, reply in Spanish. If it is in French, reply in French. If it mixes languages, match the dominant language of the target. Never translate the target to English. Never reply in English unless the target itself is in English. The tone, length, and platform rules that follow apply to the reply you write in the target's language; they do not change the language itself.

Output only the reply text. No preamble, no quotes, no markdown headings, no signature, no language tags. Do not use em-dashes; use periods or commas instead.`;
...
export function buildUserMessage(ctx) {
  ...
  return `Context:\n\`\`\`json\n${ctxJson}\n\`\`\`\n\nReply to the 'target' ${noun}. Write the reply in the same language as the target's text — do not translate.`;
}
